...
class TestPerEventScheduler:
    def _two_event_config(self, **overrides):
        events = overrides.pop(
            "events",
            [
                EventConfig(event_id="tuesday", name="Tuesday", date="2026-07-28", url="http://tuesday"),
                EventConfig(event_id="wednesday", name="Wednesday", date="2026-07-29", url="http://wednesday"),
            ],
        )
        defaults = dict(
            events=events,
    ...
    def test_blocked_event_gets_longer_cooldown(self, tmp_path):
        scheduler = _make_scheduler(
            ...
            self._two_event_config(
                events=[
                    EventConfig(event_id="event-1", name="Night 1", date="2026-07-28", url="http://event")
                ],
